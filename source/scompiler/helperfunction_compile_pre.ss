...
(define pre_eval_global 
  (lambda () 
    ...
    (cond
      ((eq? output_method 'HIP)
	(begin
	  (set! kernel_fun_prefix '__global__)
	  (set! device_fun_prefix '__device__)
	  (set! kernel_arg_prefix ""))
	(set! g_idx '(+ hipThreadIdx_x (* hipThreadIdx_y hipBlockDim_x)))
	(set! g_idy '(+ hipBlockIdx_x (* hipBlockIdx_y hipGridDim_x)))
	(set! g_xlen '(* hipBlockDim_x hipBlockDim_y))
	(set! g_ylen '(* hipGridDim_x hipGridDim_y))
	...
	(set! g_global_idx '(+ __idx (* __idy __xlen)))
      ...
      ((eq? output_method 'SYCL)
	(set! g_global_idx '(scmc_local_global_id.get_global_id 0))
	(set! g_idx '(scmc_local_global_id.get_local_id 0))
	(set! g_idy `(/ ,g_global_idx ,g_idx))
	(set! g_ylen '__scmc_internal_ylen)
	(set! g_xlen '__scmc_internal_xlen)
	)
   ...
   (cond 
      ((or (isinlst output_method '(CUDA OpenCL HIP)))
