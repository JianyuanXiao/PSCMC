...
(defmacro import (str . rest)
  (define str-lst (string->list str))
  (define new-name-str 
    (let loop ((str-lst (reverse str-lst)) (ccx #f))
      (cond
	((and (or (null? str-lst) (eq? (car str-lst) #\/)) (not (eq? #f ccx))) (list->string ccx))
	((not (eq? ccx #f))
	  (loop (cdr str-lst) (cons (car str-lst) ccx))
	  )
	((eq? (car str-lst) #\.)
	  (loop (cdr str-lst) '())
	  )
	(else
	  (loop (cdr str-lst) ccx)
	  )
	)
      ))
  (if (null? rest)
    (set! rest (string->symbol new-name-str))
    (set! rest (car rest))
    )
  `(import-core ,str ,rest)
  )
(defmacro import-core (str asname)
  (define expanded-expr (macroexpand-all (optload-core str)))
  `(begin
     (define ,asname 
       (let () 
	 ,expanded-expr
	   (lambda (callname) (eval callname))
	 )
       )
     (defmacro ,(multi-concat asname '-ref) (var)
       (list ',asname (list 'quote var))
       )
     (defmacro ,(multi-concat asname '-set!) (var val)
       (list ',asname (list 'quote (list 'set! var val)))
       )
     )
  )
 ...
 (define (quick-sort-core vec beg end1)
  (cond
   ...
   (else 
    ...
    (let loop ()
      (cfor 'done (and (< i end1) (cmpfun (vector-ref vec i) midvar)) (set! i (+ i 1)) 
	(if #f (begin (write 'i=)(write i) (newline)) 0)
	) 
      (cfor 'done (and (> j 0) (not (cmpfun (vector-ref vec j) midvar))) 
	(set! j (- j 1)) 
	(if #f (begin (write 'j=)(write j) (newline)) 0))
     (cond
      ((>= i j) 'done)
      (else 
